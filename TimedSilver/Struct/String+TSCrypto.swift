...
extension String {
    var ts_md5String: String {
        if let data = dataUsingEncoding(NSUTF8StringEncoding) {
            let MD5Calculator = TSMD5(Array(UnsafeBufferPointer(start: UnsafePointer<UInt8>(data.bytes), count: data.length)))
...
func ts_arrayOfBytes<T>(value: T, length: Int? = nil) -> [UInt8] {
...
extension Int {
    ...
    func ts_bytes(totalBytes: Int = sizeof(Int)) -> [UInt8] {
...
extension NSMutableData {
    ...
    func ts_appendBytes(ts_arrayOfBytes: [UInt8]) {
        appendBytes(ts_arrayOfBytes, length: ts_arrayOfBytes.count)
    }
    
}

protocol TSHashProtocol {
...
func ts_toUInt32Array(slice: ArraySlice<UInt8>) -> Array<UInt32> {
...
struct ts_BytesSequence: SequenceType {
    let chunkSize: Int
    let data: [UInt8]
...
func ts_rotateLeft(value: UInt32, bits: UInt32) -> UInt32 {
    return ((value << bits) & 0xFFFFFFFF) | (value >> (32 - bits))
}

class TSMD5: TSHashProtocol {
    ...
    func calculate() -> [UInt8] {
        ...
        let lengthBytes = lengthInBits.ts_bytes(64 / 8)
        tmpMessage += lengthBytes.reverse()
        
        // Process the message in successive 512-bit chunks:
        let chunkSizeBytes = 512 / 8 // 64
        
        for chunk in ts_BytesSequence(chunkSize: chunkSizeBytes, data: tmpMessage) {
            // break chunk into sixteen 32-bit words M[j], 0 ≤ j ≤ 15
            var M = ts_toUInt32Array(chunk)
            ...
            for j in 0 ..< sines.count {
                ...
                D = C
                C = B
                B = B &+ ts_rotateLeft((A &+ F &+ sines[j] &+ M[g]), bits: shifts[j])
                A = dTemp
            }
